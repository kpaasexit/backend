...
from app.grpc_service.question_servicer import QuestionServicer
...
class QuestionGRPCServer:
    def __init__(self):
        self.settings = get_settings()
        self.logger = LoggerSetup.get_logger(__name__)
        self.server: Optional[grpc.Server] = None
        self.port = self.settings.server.question_service_port
        self._shutdown = False

    def _create_server(self) -> grpc.Server:
        thread_pool = futures.ThreadPoolExecutor(
            max_workers=10,
            thread_name_prefix="question_grpc"
        )
    ...
    def start(self) -> None:
        ...
        self.server.start()
        self.logger.info(f"Question gRPC server started on port {self.port}")
    ...
    def stop(self, grace_period: int = 30) -> None:
        ...
        self.logger.info("Stopping Question gRPC server...")
        self._shutdown = True

        self.server.stop(grace_period)
        self.logger.info("Question gRPC server stopped")
...
def create_question_grpc_server() -> QuestionGRPCServer:
    if question_service_pb2_grpc is None:
        raise ImportError(
            "gRPC code not generated. Run: ./scripts/compile_protos.sh"
        )

    return QuestionGRPCServer()


def run_question_grpc_server():
    server = create_question_grpc_server()
    server.start()
    asyncio.run(asyncio.Event().wait())


async def run_question_grpc_server_async():
    server = create_question_grpc_server()
